#!/usr/bin/env python3
# coding: utf-8
# (File name: annual-outflow01.py)
# Author: SENOO, Ken
# License: CC0
# (Last update: 2015-03-10T17:46+09:00)
...
for f_i, fr in enumerate(glob.glob(IN_DIR+"damflow01.dat")):
...
for f_i, fr in enumerate(glob.glob(IN_DIR+"damflowp01.dat")):
    dam_fr = get_variable(fr)
...
for f_i, fr in enumerate(glob.glob(IN_DIR+"damflows01.dat")):
    dam_fr = get_variable(fr)
...
for f_i, (fr_s, fr_p) in enumerate(zip(glob.glob(IN_DIR+"damflows01.dat"), glob.glob(IN_DIR+"damflowp01.dat"))):
