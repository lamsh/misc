...
"""
This program is make landuse distribution map from WPS geogrid geo_em.d0?.nc.
"""
...
for var in varlist_mcip:
    plt.clf()
    m.drawcoastlines(color="k",linewidth=0.5); m.drawcountries(color="w",linewidth=0.6)
...
#    plt.colorbar(pad=0.01,fraction=0.1,ticks=range(1,geo_nc.NUM_LAND_CAT)).ax.tick_params(labelsize=14)
    plt.colorbar(pad=0.01,fraction=0.1,ticks=range(geo_nc.NUM_LAND_CAT+1)).set_ticklabels(range(1,geo_nc.NUM_LAND_CAT+1))
#    plt.clim(1,24)
    title="USGS Land Use Category Distribution"
